...
import {
  ...
  ShoppingBag, Music, Wifi, CheckCircle2, LogOut, Landmark,
} from 'lucide-react';
...
const OnboardingScreen: React.FC<OnboardingScreenProps> = ({ onComplete }) => {
  const [step, setStep] = useState(0); // 0=Income, 1=Subs, 2=Expenses
  ...
  const submitBanks = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to save bank accounts');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0E1116] text-white px-5 py-8 overflow-y-auto relative">
      ...
      <AnimatePresence mode="wait">
        ...
        {/* ═══════ STEP 3: BANK ACCOUNTS (Linked Liquidity) ═══════ */}
        {step === 3 && (
          <motion.div
            key="banks"
            initial={{ opacity: 0, x: 40 }}
          ...
          >
            ...
            <p className="text-slate-400 text-sm mb-6">
              Add your bank accounts to enable Linked Liquidity tracking on your dashboard.
            </p>
              ...
              <div className="space-y-2 mb-6">
                ...
                <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-xl px-4 py-3 mt-3">
                  ...
                  <p className="text-lg font-bold text-emerald-300">
                    ₹{banks.reduce((sum, b) => sum + b.balance, 0).toLocaleString()}
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="flex gap-3">
              ...
              >
                {loading ? <Loader2 className="animate-spin" size={20} /> : (
                  <>{banks.length === 0 ? 'Skip & Finish' : 'Activate Capital Engine'} <Zap size={20} /></>
                )}
              </button>
            </div>
          </motion.div>
